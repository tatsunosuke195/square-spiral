...
const styles = {
  ...
  speedChipActive: {
    ...
    boxShadow: "0 0 0 1px rgba(248,250,252,0.15), 0 6px 18px rgba(2,6,23,0.28)",
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      if (lastStepRef.current === null) lastStepRef.current = time;
      ...
      if (time - lastStepRef.current >= SPEED_OPTIONS[speedMode]) {
        ...
        setPath((prevPath) => {
          ...
          const hitsPath = prevPath.some((cell) => cell.x === next.x && cell.y === next.y);
    ...
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        <div style={styles.hud}>
          ...
          <div style={styles.statCard}>
            <div style={styles.statLabel}>{speedMode === "slow" ? "Slow ハイスコア" : speedMode === "normal" ? "Normal ハイスコア" : "Fast ハイスコア"}</div>
            <div style={styles.statValue}>{highScores[speedMode]}</div>
          </div>
          <div style={styles.statCard}>
            <div style={styles.statLabel}>フィールド</div>
            <div style={styles.statValue}>{COLS}×{ROWS}</div>
          </div>
        </div>
        ...
        <div style={styles.speedRow}>
          {(["slow", "normal", "fast"] as SpeedMode[]).map((mode) => (
            ...
            >
              {mode === "slow" ? "Slow" : mode === "normal" ? "Normal" : "Fast"}
            </button>
          ))}
        </div>
